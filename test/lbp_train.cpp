...
void train_svm_hog()
{
	...
	for (int num = 0; num < PosSamNO && getline(finPos, ImgName); num++)
	{
		...
		//Calcualte LBP image
		LBP_descriptors = HIST_OLBP<char>(image);
		...
		if (0 == num)
		{
			...
			sampleFeatureMat = cv::Mat::zeros(PosSamNO + NegSamNO, LBPDescriptorDim, CV_32FC1);
		...
		for (int i = 0; i < LBPDescriptorDim; i++)
		{
			 //The ith element in the feature vector of the num sample 
			sampleFeatureMat.at<float>(num, i) = LBP_descriptors[i];
	...
	for (int num = 0; num < NegSamNO && getline(finNeg, ImgName); num++)
	{
		...
		//LBP descriptor vector
		std::vector<float> LBP_descriptors;
		//Calcualte LBP histogram
		LBP_descriptors = HIST_OLBP<char>(src);
		...
		for (int i = 0; i < LBPDescriptorDim; i++)
		{
			...
			sampleFeatureMat.at<float>(num + PosSamNO, i) = LBP_descriptors[i];
		}
